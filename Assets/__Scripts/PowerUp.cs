...
public class PowerUp : MonoBehaviour {
    ...
    //Необычное, но удобное применение Vector2.x хранит минимальное значение, а у - максимальное значение для 
    //метода Random.Range(), который будет вызываться позже
    public Vector2 rotMinMax = new Vector2(15, 90);
    public Vector2 driftMinMax = new Vector2(.25f, 2);
    public float lifeTime = 6f; //Время в секундах существования PowerUp
    public float fadeTime = 4f; //Секунд до появления, затем исчезает
    ...
    void Awake()
    {
        ...
        //Random.onUnitSphere возвращает вектор , указывающий на случайную точку, находящуюся на поверхности сферы
        //с радиусом в 1м и с центром в начале координат
        vel.z = 0; //Отобразить vel на плоскость XY
        vel.Normalize(); //Нормализация устанавлиевает длину Vector3 равной 1м
        vel *= Random.Range(driftMinMax.x, driftMinMax.y);
        rigid.velocity = vel;

        //Установить угол поворота этого игрового обхекта равным R:[0, 0, 0]
        transform.rotation = Quaternion.identity;
        //Quaternion.identity равноценно отсутсвию поворота
    ...
    void Update()
    {
        ...
        //Эффект растворение куба PowerUp с течением времени. Со значениями по умолчанию бонус существует 10 секунд,
    ...
    public void SetType (WeaponType wt)
    {
        ...
        //letter.color = def.color; //Буква тоже можно окрасить в тот же цвет
    ...
    public void AbsorbedBy (GameObject target)
    {
        ...
        //Можжно было бы реализовать эффект поглощения бонуса, уменьшая его размеры в течение нескольких кадров, но пока просто уничтожим this.gameObject
        Destroy(this.gameObject);
    }
}
